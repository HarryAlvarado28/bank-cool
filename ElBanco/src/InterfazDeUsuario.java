...
//	Image miIcono = mipantalla.getImage("img/iconBank.png");
	...
	// ---------------------------------------------------------------------------------
	// private URL ruta_externa =
	// InterfazDeUsuario.class.getResource("iconBank.png");
...
//		paraPantallas.add(jtaPantallaPrincipal, BorderLayout.CENTER);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == jbBalance) {
			...
			new EditaSaldoUsuario(nombre, apellido, usuario, numCuenta, saldoEnBanco, contrasena, tipoCuenta);
	...
	private String acerca_de = "\n\n\t\t~~ DMG BANK  ~~\n\n" + " Bank DMG es un sistema bancario disenado\n"
	...
	private double transaccion(String s) {
		try {
			...
			JOptionPane.showMessageDialog(this, "Estas por " + queHago + " B/. " + saldoFire, "Bank ATM",
					JOptionPane.INFORMATION_MESSAGE);

		} catch (Exception x) {
			JOptionPane.showMessageDialog(this, "Asegurate solo tener\n un punto y no usar coma", "Error de Valor",
		...
		if (s.equalsIgnoreCase("depositar")) {
			...
			if (saldoEnBanco < saldoFire) {
				JOptionPane.showMessageDialog(this, "Saldo insuficiente ", "Error al Retirar",
						JOptionPane.ERROR_MESSAGE);
				retiro = false;
			} else {
				saldoEnBanco -= saldoFire;
		...
		/*
		 * Le asigana a la variable saldoBancario el saldo actual del usuario para que
		 * pueda llegar a ser retornado si se le es invocado de alguna otra parte del
		 * programa.
		 */
	...
	class EditaSaldoUsuario {
		...
		public void cargarDatos(String userID) {

			listaDatos = new String[15]; // Variable que almacena los datos de laconsulta
		...
		@SuppressWarnings("unchecked")
		private void reemplazar(int index, String nombre, String apellido, String usuario, String numCuenta,
			...
			try {
				ObjectOutputStream escribiendo_fichero = new ObjectOutputStream(
						new FileOutputStream("clientesBaseDatos.txt"));
